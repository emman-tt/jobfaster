...
export default function JobBoardDetail({ job, onSave, onDelete, isSaving, isDeleting }) {
  ...
  return (
    <div className={`w-full h-full flex flex-col ${appearance.theme === 'dark' ? 'bg-[#2a2a2a] text-white' : 'bg-white text-slate-900'}`}>
      ...
      {/* Description */}
      <div className="px-6 flex-1 overflow-hidden flex flex-col gap-2 pb-4">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider ml-1">Description</p>
        <textarea
          name="jobDescription"
          value={formData.jobDescription}
          onChange={handleChange}
          className={`w-full flex-1 resize-none leading-relaxed ${inputClass()}`}
