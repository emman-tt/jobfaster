...
export function Topbar() {
      ...
      if (result.status === "success") {
        ...
        toast.error("Failed to save resume", {
          ...toastPresets.generalError(
            result.message || "Failed to save resume",
          ),
        });
      }
    } catch (error) {
  ...
  async function handleExport(format) {
    ...
    try {
      const resumeHTML = await generatePaginatedResumeHTML(
        rawData,
        templateId,
        { styles },
      );
      ...
      if (!printWindow) {
        toast.error("Popup blocked", {
          ...toastPresets.generalError(
            "Please allow popups to export your resume",
          ),
        });
        return;
      }
      ...
      >
        <div className="relative  ">
          ...
          {menuBar && (
            <div
              ref={menuBarRef}
              className="absolute top-full left-0 right-0 mt-5 w-120 max-sm:w-[calc(100vw-2rem)] max-sm:left-2 z-100"
            >
              <Menubar />
            </div>
          )}
