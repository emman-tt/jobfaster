...
const CreativePortfolio = ({ data }) => {
  ...
  return (
    ...
    >
      {data?.pageNumber > 1 && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, marginBottom: '12px' }}>
      ...
      >
        ...
        <div style={{ borderTop: `2px solid ${accentColor}`, margin: '16px 0' }} />

        {data?.phone && (
          <div style={{ marginBottom: '8px', fontSize: `${contactStyles.size || 9}pt` }}>
        ...
        {data?.email && (
          <div style={{ marginBottom: '8px', fontSize: `${contactStyles.size || 9}pt` }}>
            ...
            <div style={{ color: '#fff' }}>{data.email}</div>
          </div>
        )}
        ...
        {links?.length > 0 && (
          <div style={{ marginTop: '16px', fontSize: `${contactStyles.size || 9}pt` }}>
            <span style={{ color: '#888' }}>Links</span>
            {links.map((item, index) => (
              <div key={item.name} style={{ color: accentColor, marginTop: '4px' }}>
                {item.link}
              </div>
            ))}
          </div>
        ...
        {data?.languages?.length > 0 && (
          <div style={{ marginTop: '24px' }}>
            ...
            {data.languages.map((lang, i) => (
              <div key={i} style={{ fontSize: '9pt', marginBottom: '4px', color: '#ccc' }}>
                {lang.name} <span style={{ color: '#888' }}>({lang.proficiency})</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ flex: 1, padding: '24px 20px' }}>
        ...
        {data?.experience?.length > 0 && (
          <div style={{ marginBottom: '20px' }}>
            ...
            {data.experience.map(exp => (
              <div key={exp.id} data-section="experience" style={{ marginBottom: '16px' }}>
                <div
                  style={{
                    display: 'flex',
        ...
        {data?.projects?.length > 0 && (
          <div style={{ marginBottom: '20px' }}>
            ...
            {data.projects.map(proj => (
              <div key={proj.id} data-section="projects" style={{ marginBottom: '12px' }}>
                <h3
                  style={{
        ...
        {(data?.education?.length > 0 || data?.certificates?.length > 0 || data?.achievements?.length > 0) && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              ...
              {data?.education?.length > 0 && (
                <>
                  ...
                  {data.education.map(edu => (
                    <div key={edu.id} data-section="education" style={{ marginBottom: '8px' }}>
                      <div style={{ fontWeight: 'bold', fontSize: `${companyStyles.size || 10}pt`, margin: '0' }}>
                        {edu.degree} {edu.field && `in ${edu.field}`}
                      </div>
                      <div style={{ fontSize: `${bodyStyles.size || 9}pt`, color: '#666', margin: '2px 0' }}>
                        {edu.school} • {edu.startYear} - {edu.endYear}
                      </div>
                      {edu.highlights?.length > 0 && (
                        <div style={{ fontSize: `${dateStyles.size || 8}pt`, color: '#888' }}>
                          {edu.highlights.join(' | ')}
                        </div>
                      )}
                    </div>
            ...
            <div>
              {data?.certificates?.length > 0 && (
                <>
                  ...
                  {data.certificates.map((cert, i) => (
                    <div key={i} style={{ fontSize: `${bodyStyles.size || 9}pt`, marginBottom: '4px' }}>
                      • {cert.name}
                    </div>
                  ))}
                </>
              ...
              {data?.achievements?.length > 0 && (
                <div style={{ marginTop: data?.certificates?.length > 0 ? '16px' : 0 }}>
                  <h2
                    style={{
                      fontSize: `${sectionStyles.size || 11}pt`,
                      fontWeight: 'bold',
