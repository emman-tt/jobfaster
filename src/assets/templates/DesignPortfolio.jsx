...
const DesignPortfolio = ({ data }) => {
  ...
  return (
    ...
    >
      {data?.pageNumber > 1 && (
        <div style={{ marginBottom: '12px', paddingBottom: '8px', borderBottom: `2px solid ${accentPink}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10pt', color: mediumGray }}>
            <span style={{ fontWeight: 'bold', color: accentPink }}>{data?.name}</span>
      ...
      >
        ...
        >
          ...
          {links?.map((item, index) => (
            <span key={item.name} style={{ color: accentPink, fontWeight: 500 }}>🔗 {item.link}</span>
          ))}
        </div>
      </div>
          ...
          {data.experience.map((exp, expIndex) => (
            <div
              ...
              style={{
                marginBottom: '20px',
                paddingLeft: '20px',
                borderLeft: expIndex < data.experience.length - 1 ? `2px solid #E4E4E7` : 'none',
                position: 'relative',
              }}
            >
              <div
              ...
              {exp.accomplishments?.map((acc, i) => (
                ...
                >
                  <span style={{ color: accentPink, fontWeight: 'bold' }}>◦</span> {acc}
                </div>
              ))}
            </div>
          ))}
            ...
            <div>
              ...
              {data.education.map(edu => (
                <div key={edu.id} data-section="education" style={{ marginBottom: '12px' }}>
                  ...
                  <div style={{ fontSize: `${bodyStyles.size || 9.5}pt`, color: '#52525B', margin: '2px 0' }}>
                    {edu.school}
                  </div>
                  <div style={{ fontSize: `${dateStyles.size || 9}pt`, color: mediumGray, margin: '0' }}>
          ...
          {data?.certificates?.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              ...
              {data.certificates.map((cert, i) => (
                ...
                >
                  <span style={{ fontWeight: 700, color: accentPink }}>✓</span> {cert.name}
                </div>
              ))}
            </div>
          )}
              ...
              <div style={{ fontSize: `${bodyStyles.size || 9.5}pt`, lineHeight: 1.8 }}>
                {data.languages.map((lang, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ color: accentPink }}>{lang.proficiency}</span>
                  </div>
                ))}
              </div>
            </div>
