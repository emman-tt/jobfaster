  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: `${contactStyles.size || 8.5}pt`, lineHeight: 1.8 }}>
            {data?.phone && (
              <div style={{ marginBottom: '4px' }}>
                <div style={{ fontWeight: 600, color: primaryBlue, marginBottom: '1px' }}>Phone</div>
                <div>{data.phone}</div>
              </div>
            )}
            {data?.email && (
              <div style={{ marginBottom: '4px' }}>
                <div style={{ fontWeight: 600, color: primaryBlue, marginBottom: '1px' }}>Email</div>
                <div>{data.email}</div>
              </div>
            )}
            {data?.location && (
              <div style={{ marginBottom: '4px' }}>
                <div style={{ fontWeight: 600, color: primaryBlue, marginBottom: '1px' }}>Location</div>
          ...
          {links?.length > 0 && (
            <div style={{ marginTop: '10px' }}>
              <div style={{ fontWeight: 600, color: primaryBlue, marginBottom: '4px', fontSize: '8.5pt' }}>Links</div>
              {links.map((item, index) => (
                <div key={item.name} style={{ marginBottom: '3px', fontSize: '8.5pt' }}>
                  {item.link}
                </div>
              ))}
            </div>
        ...
        {data?.skills?.length > 0 && (
          ...
          >
            ...
            <div style={{ fontSize: `${bodyStyles.size || 8.5}pt`, lineHeight: 1.8 }}>
              {data.skills.map((skill, i) => (
                <div key={i} style={{ marginBottom: '2px', display: 'flex', alignItems: 'center' }}>
                  <span style={{ width: '6px', height: '6px', background: primaryBlue, borderRadius: '50%', marginRight: '8px', flexShrink: 0 }} />
                  {skill}
                </div>
              ))}
            </div>
            ...
            <div style={{ fontSize: `${bodyStyles.size || 8.5}pt`, lineHeight: 1.6 }}>
              {data.certificates.map((cert, i) => (
                <div key={i} style={{ marginBottom: '4px' }}>
                  <span style={{ color: primaryBlue, fontWeight: 'bold' }}>✓</span> {cert.name}
                </div>
              ))}
            </div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: `${bodyStyles.size || 8.5}pt`, lineHeight: 1.7 }}>
              {data.languages.map((lang, i) => (
                <div key={i} style={{ marginBottom: '3px', display: 'flex', justifyContent: 'space-between' }}>
        ...
        {data?.education?.length > 0 && (
          ...
          >
            ...
            {data.education.map((edu, i) => (
              <div key={edu.id} style={{ marginBottom: '12px', fontSize: '8.5pt' }}>
      ...
      <div style={{ flex: 1, padding: '28px 28px' }}>
        {data?.pageNumber > 1 && (
          <div style={{ marginBottom: '16px', paddingBottom: '8px', borderBottom: `2px solid ${primaryBlue}` }}>
            ...
            {data.experience.map(exp => (
              ...
              >
                ...
                {exp.accomplishments?.map((acc, i) => (
                  ...
                  >
                    <span style={{ color: primaryBlue, fontWeight: 'bold', marginRight: '6px' }}>•</span> {acc}
                  </div>
                ))}
              </div>
            ))}
            ...
            {data.projects.map(proj => (
              ...
              >
                ...
                {proj.techStack?.length > 0 && (
                  ...
                  >
                    <span style={{ fontWeight: 600 }}>Technologies:</span> {proj.techStack.join(', ')}
                  </p>
                )}
              </div>
            ))}
